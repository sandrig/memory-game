...
export let setStatusAt = (i: number) => (status: Cell.Status) => (
  board: Board,
): Board => R.set(R.lensPath(`${i}.status`), status, board)

export let getStatusesBy = (predFn: Cell.PredFn) => (
  board: Board,
): Cell.Status[] =>
  R.chain((cell: Cell.Cell) => (predFn(cell) ? [cell.status] : []), board)

export let setStatusesBy = (predFn: Cell.PredFn) => (status: Cell.Status) => (
  board: Board,
): Board =>
